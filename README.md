...
- In the RepairManager class, which implements the Manageable interface and therefore inherits its required method structure. By implementing this interface, RepairManager provide its own version of add(), view(), update(), and delete(), making it consistent with any other class that follows the same pattern. The Repair class also implicitly inherits from Java’s built-in Object class, giving it core behaviors such as toString(). This inherited method is overridden in your code to provide a customized string output for each repair.
